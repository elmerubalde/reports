...
const data = ref( { labels: [], datasets: [
                { label: 'LG Member', backgroundColor: '#3B82F6', data: [] }, 
                { label: 'LG Attendance', backgroundColor: '#5310b9', data: [] } 
              ] }); 
...
const fetchMembership = async (endDate: date) => { 
  ...
  try {
    ...
    data.value = await auth.apiFetch(`/api/report/lgmembershipvsattendance/${dtStart}/${dtEnd}`) 
  } catch (err) {
